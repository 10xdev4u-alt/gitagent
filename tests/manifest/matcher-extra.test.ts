...
function makeManifest(name: string, triggers: string[]): Manifest {
  return {
    frontmatter: {
      ...
      memory: { type: 'git', path: 'memory' },
      tools: [],
      approval: { read: 'never', write: 'required' },
      limits: { maxSteps: 10, timeoutMs: 60000 },
      permissions: { closeIssues: false, mergePRs: false, release: false },
    },
    body: '',
    path: `${name}.md`,
  };
